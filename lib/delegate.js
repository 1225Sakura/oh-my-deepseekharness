...
import { TIER_ORDER, buildRoutingTable, routeOnce, recordFallback } from './routing.js'
...
/** c3 路由日志行同款格式（routing.js fmtLog 镜像，格式漂移由 routing 测试钉住）。 */
function fmtRouteLog(d) {
  return `[routing] dispatch=${d.dispatchId} role=${d.role ?? 'none'} table=${d.tableTier ?? 'miss'}`
    ...
    async execute(args, exec) {
      ...
      if (modelId !== undefined) {
        ...
        if (!llm || typeof llm.resolveCallConfig !== 'function')
          throw new Error(`omd_delegate: llm 服务缺席——模型 id "${modelId}" 无法校验（不静默派发）`)
        providerId = args.provider ?? parentRouteForDelegation(parent).provider
        if (providerId === undefined)
          throw new Error(`omd_delegate: 无法确定 LLM provider（调用代理无路由且未显式提供）——model=${modelId}`)
        await llm.resolveCallConfig({ provider: providerId, model: modelId }, exec?.signal)
        agentOptions = { provider: providerId, model: modelId }
      }
      ...
      // c3 fallback 审计（派发前判定面；审计失败仅告警不阻断派发）
      if (decision.fallback) {
        try {
          await recordFallback({ stateDir: stateRoot(), decision })
        } catch (e) {
