...
> 接线面注（2026-09-22 评审对齐）：c4 HUD 摘要卡（lib/hud.js）库能力还原——v0.3.0 commit 9 后 lib/hud.js 与 tests/lib/hud.test.js 已上线（七要素契约：mode/round/story/agents/todo），但 0.x 范围**未创建 client half**，因此用户在 dsh Web UI 上仍看不到持续面板；现可经 mcp__omd-state__state_get_status + notepad_stats 派生五要素，模型在助手消息里调用 renderCard 渲染 6 行有界文本作为「最小 HUD 实现」。client half（仿 dsh-better-sidebar 的 ctx.betterSidebar 形态注册原生 tab）属 1.x 路线。c5/c6/c8 由 \`mcp__omd-state__team_*\` 工具面接线（team_begin / team_dispose / team_scan_orphans / team_write_mirror / team_write_tree_state）。已接线生效：c1 omd_delegate、c2 keyword-hook（agent/pre-step）、c3 routeOnce 路由、c6 fallback 审计写入。
