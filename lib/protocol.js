...
### 关键词 hook（原生拦截点，M3 c2 — 唯一一根 hook 电线）
...
- **hook 矩阵范围（诚实声明）**：当前实现仅 c2 keyword-hook 一根电线；OMC 5.5.0 的 30+ hook 矩阵（agent-usage-reminder / auto-slash-command / background-notification / beads-context / code-simplifier / comment-checker / directory-readme-injector / empty-message-sanitizer / factcheck / jev / learner / merge-readiness / mode-registry / non-interactive-env / permission-handler / persistent-mode / plugin-patterns / pre-compact / preemptive-compaction / prompt-prerequisites / recovery / registry / rules-injector / self-improve / session-end / setup / skill-state / task-size-detector / team-pipeline / think-mode / thinking-block-validator / todo-continuation / autopilot / ralph）**未实现**——dsh 宿主事件面除 \`agent/pre-step\` 外的可用事件清单未在 0.x 探测，OMC 风格的事件钩子在 omd 0.x 范围外
...
- 每次派发恰记一行 \`[routing] dispatch=<id>\` 日志（路由:派发=1:1）；表外角色落会话默认档 ${config.routing?.defaultTier ?? 'medium'}，且 fallback 审计记入权威 run-state（\`${config.stateDir ?? '.omd'}/state/run-state.json\`，可审计）

> 接线面注（v0.4.0 对齐）：c4 HUD 已全量落地——server 面 \`mcp__omd-state__hud_render / hud_summary\`（五要素 6 行卡，数据源 state_get_status+notepad_stats）+ client half（lib/client.js 注册右侧边栏 tab 类型 'omd-hud'，数据面 /oh-my-dsh/hud.json 经 webServer exact 路由；CLI 会话 hudRoute 记 skipped）。team 领队运行时（lib/team.js）已上线：阶段状态机 team_phase_transition（fix 循环 ≤3 由状态机强制）、队员 UUID 生命周期 team_register_worker/team_worker_update/team_registry（runId=owner-epoch，终态不可复活）、mailbox team_mail_*、heartbeat team_heartbeat_scan（+插件侧周期检测定时器 Config team.heartbeatIntervalMs；检测+呈面，催促归领队模型）、merge 计划 team_merge_plan。c5/c6/c8 工具面同上（team_begin/dispose/scan_orphans/write_mirror/write_tree_state）。代码智能 MCP 已上线：ast_grep_search/ast_grep_replace（@ast-grep/napi 缺席显式报错）+ lsp_start/stop/status/document_symbols/references/definition/diagnostics（Config codeIntel.lspServers 注册表驱动，单实例/崩溃重启≤2）。trace 数据面已上线：trace_begin/trace_event/trace_summary/trace_list（.omd/trace/<id>.jsonl）。已接线生效：c1 omd_delegate、c2 keyword-hook（agent/pre-step）、c3 routeOnce 路由、c6 fallback 审计写入。

### c5 team worktree 隔离（每运行一树；两阶段拆除；孤儿绝不自动删）
- 队长 run 启动 → \`mcp__omd-state__team_begin({ cwd, runId, stateDir })\` → 建 ${config.stateDir ?? '.omd'}/worktrees/{run-id}（gitignored，从当前 HEAD \`git worktree add --detach\`），启动时先扫孤儿：run-state 非终态且无活跃会话 → 只提示处置，**绝不自动删**
- 两阶段拆除：\`mcp__omd-state__team_dispose({ cwd, runId, reason, expectUpdatedAt })\` → 阶段①run-state 先 CAS 写 disposed 终态 → 阶段②再删树（终态时间戳必须早于删树时间；CAS 不成功绝不删树）
- 用户取消：删树前先列树内与 HEAD 差异文件（只提示不阻塞）
- 崩溃恢复：\`mcp__omd-state__team_scan_orphans({ cwd })\` 在 team_begin 启动前调一次（lib 内部自动）

### c6 run-state 镜像契约（七字段快照 CAS；队长独占写）
- run-state.json 权威=<cwd>/${config.stateDir ?? '.omd'}/state/run-state.json（运行时：cwd 为工作区绝对路径，stateDir 为配置项），七字段镜像：mode/round/current_story/active_agents/todo（计数+首条标题）+version+updatedAt；version 1 起步每写 +1
- **队长独占写**（队员只读）；两写时机契约：阶段边界（team_begin / team_dispose）+ 阶段过渡（每次 stage transition）
- **MCP 工具**：\`mcp__omd-state__team_write_mirror({ cwd, stateDir, patch, expectUpdatedAt, retries })\` — patch 经 \`buildMirrorPatch\` 构造五要素镜像键，CAS 冲突重读最新值重试 ≤3 次，预算耗尽覆盖写 + warning 日志行（全程 version 严格 +1）

### c8 多仓锚定（树内懒建；豁免向上 marker 解析；残渣随树丢弃）
- **MCP 工具**：\`mcp__omd-state__team_write_tree_state({ cwd, rel, data, stateDir })\` — 仅在 c5 运行树内可用（cwd 在 ${config.stateDir ?? '.omd'}/worktrees/{run-id} 下）；非运行树抛错
- 树内写 omd 状态 → 树内 \`${config.stateDir ?? '.omd'}/\` **首次写才懒创建**，父仓状态树零新增写入（不外溢；父仓 .omd 基线 hash 前后一致）
