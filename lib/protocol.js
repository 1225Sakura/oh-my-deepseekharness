...
> 接线面注（2026-09-22 评审对齐）：c4/c5/c6/c8 已交付为**库能力**（lib/hud|worktree|runstate|multirepo，测试在案）；其工具面/流水线接线在后续里程碑——以下小节描述行为契约与验收判据，不断言接线已完成。已接线生效：c2 keyword-hook（agent/pre-step）、c3 omd_delegate 路由、c6 fallback 审计写入。
...
- **队长独占写**（队员只读）；两写时机契约：阶段边界（run 启动/拆除）+ 派发前快照（与 c3 routeOnce 同临界区——契约面；delegate 接线在后续里程碑，当前 delegate 仅写 fallback 审计面）
