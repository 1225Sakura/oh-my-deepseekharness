...
> 接线面注（2026-09-22 评审对齐）：c4 HUD 摘要卡（lib/hud.js）已下线——自检发现系统提示词 context 不支持轮询，HUD 形态与 dsh 宿主装配语义不匹配，删除以避免误导。c5/c6/c8 由 \`mcp__omd-state__team_*\` 工具面接线（team_begin / team_dispose / team_scan_orphans / team_write_mirror / team_write_tree_state）。已接线生效：c1 omd_delegate、c2 keyword-hook（agent/pre-step）、c3 routeOnce 路由、c6 fallback 审计写入。
