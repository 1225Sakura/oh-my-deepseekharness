...
/**
 * F3 热修（2026-09-21，用户 T4②-C 授权）：schemastery 的 z.string() 在部署环境
 * 没有 .parse —— dsh-storage-domain 的域加载路径（lib/index.js:371
 * valueSchema.parse(raw)）对任何非空 memory 表抛 TypeError → 实例启动即 fatal
 * （S2 spike 判别实验：17 字符 ASCII 记录即炸；写路径不校验，故只在重启时引爆）。
 * domainTable 只消费 valueSchema.parse(raw)，故传一个最小 zod 兼容 shim：
 * 字符串放行原样返回，非字符串拒绝（拒绝由 domain 层包装为 invalid-record，
 * 不静默；配合 spec `invalidRecords: 'backup-and-skip'` 可进一步降级为备份跳过）。
 */
export function memoryRecordSchema() {
  return {
    parse(raw) {
      if (typeof raw !== 'string') {
        throw new TypeError(`memory record must be a string, got ${typeof raw}`)
      }
      return raw
    },
  }
...
export async function registerMemoryTools(ctx, defineTool, deps = {}) {
  ...
  const MemoryDomain = defineDomain({
    name: 'oh_my_dsh',
    version: 0,
    tables: { memory: domainTable(memoryRecordSchema()) },
  })
