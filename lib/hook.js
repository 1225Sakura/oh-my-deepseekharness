  ...
  for (const entry of KEYWORD_REGISTRY) {
    ...
    if (policy.match === 'anchored') {
      ...
      for (const re of pats) {
        const m = re.exec(s)
        if (!m) continue
        // 否定前导守卫（B1 复审）：锚点前 8 字符含疑问词 = 解释性提及，交模型层判定
        const lead = s.slice(Math.max(0, m.index - 8), m.index)
 ...
 * tool 来源（防御纵深——宿主工具结果消息同为 role='user'，以 source.kind 区分，
 * role 过滤挡不住 tool 形态，故双保险）。语义守卫主防线=锚定形态+否定前导守卫。
 */
function flattenText(messages) {
  return messages
    .filter(m => m?.role === undefined || m.role === 'user')
...
export async function registerKeywordHook(ctx, opts = {}) {
  ...
  ctx.on('agent/pre-step', async ({ agent, messages }, next) => {
    ...
    if (appended && downstream && downstream.kind === 'enter') {
      const context = createUserMessage({ content: [{ type: 'text', text: appended }], source: HOOK_SOURCE })
