...
when-to-use: The user asks for a minimal, lean, YAGNI, or smallest-possible-diff implementation; a plan or in-flight change is growing beyond the stated request; writing new code where scope creep, reinvented helpers, or speculative generality are likely; sizing a proposed diff before implementation.
---
...
**Reuse before writing.** Search the codebase (`grep`/`glob`) for an existing helper, type, or pattern before writing anything new. Never copy a helper that already lives a few files away — reuse it or extract it to one shared location.
...
Minimal-code-discipline **holds no mode state**. It is a writing-time discipline, not an execution mode: no `state_write`/`state_clear`, no files under `.omd/`. When applied inside an enclosing mode (autopilot, ralph, team), persistence belongs to that mode's state contract.
