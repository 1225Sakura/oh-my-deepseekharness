...
## 会话边界清单（显式挂载/落盘——替代 OMC wiki session hook）

dsh 没有 session-start / session-end hook，wiki 的「自动挂载、自动落盘」不会自己发生。用下面两张**模型驱动的显式清单**替代（学到重要东西时立刻执行，别等会话结束）：

**会话开始 / 任务定向时（挂载）**：
1. 读 `.omd/wiki/index.md` 看有没有与任务相关的分类/页面。
2. 有命中 → `grep` 标签/关键词，读相关页，把已有结论带进当前工作，别重新发明。
3. 无 wiki 目录 → 跳过（不要为挂载而创建空库）。

**重大发现落地时 / 会话收尾前（落盘）**：
1. 该发现跨会话有用吗？是 → 按「操作」表 wiki-add/ingest 成页面（优先更新已有页）。
2. 每次写入同步维护 `index.md` 与 `log.md`。
3. 无沉淀项时显式声明一句，不许静默跳过。

确定性的 hook 自动捕获仍是二期项（依赖宿主开放会话事件面）；在那之前这两张清单就是捕获机制。
