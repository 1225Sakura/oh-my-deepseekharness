...
- **Actionable / high-confidence findings hard-block the run:** list every such finding verbatim, point at the `drydock` skill, state that the run never started and no artifacts were produced, and stop. Without an explicit override, treat any finding as blocking.
...
Launch is a **stateless composition over omd's existing lifecycle** — it owns no runtime state machine:

- The omd `team` skill owns worker coordination, handoffs, and cancellation; Launch never mutates team state outside team's contract.
- The canonical `plan` → `execute` → `review` → `verify` surfaces own their existing lifecycle behavior. Launch-authored artifacts are limited to `.omd/specs/<feature-slug>/`, `CONTEXT.md`, `docs/adr/`, and `docs/business/` — plus, only after C5 approval, the sediment slots named in the Phase 5 table.
- Launch has no automatic resume. After interruption, re-read the artifacts and current team status, but continue only through a new explicit Launch invocation after the owning team run has reached a closed boundary. Never infer a human approval or replay an in-progress worker.
- Foggy efforts are not launch's jurisdiction: when the fog gate stops the run, the run never started — no artifacts, no partial state. omd has no navigator skill; converge the destination with `deep-interview` or `plan` and come back with a sharpened mission brief.
...
Run the interview with the design-tree protocol: map decisions and their dependencies, then work in **frontier rounds** — batch every currently-askable question into one round (use `ask_user_question`), numbered, each with a recommended answer. The human answers; the tree reshapes; recompute the frontier. Facts are always self-served by sub-agents from repo evidence (`omd-agent-explore` via `subagent`) — the human is asked only what no amount of exploration can settle.
...
Split into vertical slices under `.omd/specs/<feature-slug>/tickets/`:
...
**Parallel (default, 2+ tickets).** Hand tickets to the omd `team` skill: the captain spawns workers via `subagent`, each ticket becomes a team-exec work item, and `blockedBy` edges are **declared up front** as the team's dependency contract — workers are dispatched only when their blockers are reported complete. Each worker implements with the tdd discipline at the seams approved in C2, applying `minimal-code-discipline` as the writing-time discipline when that skill is loaded (it stays opt-in); a ticket closes only after code-review passes on the diff, declared by the reviewer (`omd-agent-code-reviewer`) — the implementer never self-approves.

**Serial (single ticket, or `--serial`).** Delegate one ticket at a time to an `omd-agent-executor` subagent; same review gate.
...
**C4 — decisions that emerge mid-run.** When a worker hits a decision passing the ADR test, it stops before decision-dependent mutation, records the question (options, recommendation, reversibility note) in its final report and in `.omd/specs/<feature-slug>/decisions-pending.md`, and ends its turn without claiming completion. This is a terminal outcome for that worker: do not respawn it to retry, do not infer the human's answer, do not start another team run from this invocation. Surface the blocker with pointers to the stopped worker's report and the decision artifact.

On a later explicit Launch invocation, first confirm the previous team run is fully closed. Then batch every pending C4 question for the human, record the answers in the decision log/ADRs, and rebuild the ticket frontier before starting execution. All ticket dependencies are declared before dispatch; the captain never dispatches a worker whose blockers are not reported complete. Launch never dynamically mutates a dispatched worker's dependencies and never promises automatic re-dispatch after C4.

**Serial C4 (`--serial`).** The executor stops before decision-dependent mutation and returns the question without claiming completion. Record and resolve the human question at the batch boundary, then start a fresh executor with the recorded answer and remaining acceptance criteria. Do not replay or resume the interrupted executor context.
...
- yard re-check: re-run the drydock `--check` audit; any new findings since entry are reported as **yard drift**, with a pointer to the `drydock` skill
- **sediment pass — answer: what did this ship teach the yard?** Sweep the source checklist first (three-strike failure root causes, C4 answers, review rejections, verify findings, and any deferred-sediment lines this run carried in), then answer. Propose every lesson as `lesson → slot → intended change` against the slot table below, or decline it explicitly with a reason; a ship with nothing to teach must say so verbatim as "no new lessons". This requirement blocks non-answers, never empty answers — inventing lessons to have one is the same violation as skipping the question. The lesson list rides in the completion report next to the Open Assumptions, each line individually vetoable; approved lessons are written to their slots only after acceptance, and the report records each landing's file location. Before writing any lesson into a slot, call the `skill` tool with `agent-doc-discipline` if loaded and apply its rules; the sediment pass is complete only after the skill's verification checklist passes.
...
- Phases 1–3 in one unbroken context window; compact at phase boundaries only (watch the context-usage figure in the session's runtime context as the high-water signal).
- Long runs: prefer background jobs with periodic progress markers so the orchestrator sees liveness — a silent foreground turn emits nothing until it ends.
- Phase 4 runs in fresh contexts per ticket by construction (team workers or subagents).
- Handoffs pass pointers, never content.
- Session died mid-run: preserve the artifacts and stop. A later explicit invocation may continue only after the previous team run is closed; the artifacts on disk are authoritative for what was decided.
...
Launch **holds no omd mode state** — it is a stateless composition: no `state_write`/`state_clear` of its own. If a `team` run is spawned under Phase 4, that run follows the `team` skill's own state contract. Launch's durability is files on disk: `.omd/specs/<feature-slug>/` (spec, tickets, decisions-pending.md), `CONTEXT.md`, `docs/adr/`, `docs/business/`. On interruption, preserve artifacts and stop; resume only via a new explicit invocation after the previous team run is closed.
