---
name: trace
description: Evidence-driven tracing lane that orchestrates competing tracer hypotheses as parallel subagents
when-to-use: Ambiguous, causal, evidence-heavy "why did this happen" questions — runtime bugs/regressions, performance behavior, postmortems, config/orchestration mysteries — where competing explanations should be explored in parallel before any fix is attempted.
---
...
This is the orchestration layer on top of the `omd-agent-tracer` role. The goal is to make tracing a reusable omd operating lane: restate the observation, generate competing explanations, gather evidence in parallel, rank the explanations, and propose the next probe that would collapse uncertainty fastest.
...
## Orchestration shape (dsh)

The OMC original used Claude built-in team mode. **omd has no team-mode primitive for this lane — the lead (main session) spawns lanes as parallel `subagent`s instead.** The lead should:
...
4. Spawn **3 tracer lanes by default** as background `subagent`s, each carrying the `omd-agent-tracer` role card
...
Each worker is a **`omd-agent-tracer` lane owner**, not a generic executor.
...
- relevant code, tests, configs, docs, logs, outputs, and benchmark artifacts (`grep` / `read` / `glob`)
- focused reproductions and one-off probes via `pwsh`
...
3. "Spawn one `omd-agent-tracer` subagent per hypothesis, in the background, in parallel."
