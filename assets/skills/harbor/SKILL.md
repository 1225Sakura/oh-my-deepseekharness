...
when-to-use: A repo receives external issues/bug reports/PRs and the maintainer is drowning in triage; you want an evidence-first intake that only escalates genuinely unresolved judgments. Requires a reachable GitHub remote (V1) and the gh CLI. Not the debugger, not the review surface, never a merger.
---
...
Verify the remote tracker responds (`gh repo view` on the recorded/implicit remote — run via the session's shell tool, `pwsh` on Windows) **and that it is the expected repository**. If it does, the remote tracker is the ONLY medium for dispositions: every sheet, label, and docket lives there. A local `issues/` directory (or any untracked markdown collection) is **content to inspect, never a tracker to write to** — chaotic repos are full of lookalike directories, and writing dispositions into local files strands them where reporters and maintainers will never see them. If the remote is unreachable, stop and report the blocker — do not fall back to local files.

**V1 scope**: remote GitHub only. Other backends are explicitly unsupported — report and stop; no silent migration, no local fallback. The tracker and label vocabulary should have been recorded by the `drydock` skill; if not, ask once and record the answer in `CLAUDE.md` under the shipyard conventions.
...
harbor **holds no mode state**: no `state_write`/`state_clear`, nothing under `.omd/`. The tracker (comments, labels, the single docket issue) is the only record source and the only resume surface — the next sweep re-reads tracker state instead of any local cache. On interruption, stop; a later invocation re-binds the tracker (Step 0) and re-reads each ship's latest state.
